...
class Type {
 ...
 public: /* TagPtr layout */
  ...
  /** * extract tag **/
  static constexpr auto TagOf(TagPtr ptr) {
    return static_cast<TAG>(to_underlying(ptr) & 0x7);
  }

  /** * make tagged pointer **/
  static constexpr auto Entag(TagPtr ptr, TAG tag) {
  ...
  /** * as an address **/
  static auto ToAddress(TagPtr ptr) {
    return reinterpret_cast<void*>(to_underlying(ptr));
  }

  /** * from uint64_t **/
  static auto FromUint64(uint64_t ptr) { return static_cast<TagPtr>(ptr); }

  /** * to uint64_t **/
  static constexpr auto ToUint64(TagPtr ptr) { return to_underlying(ptr); }
  ...
  static constexpr auto ImmediateData(TagPtr ptr) {
    return static_cast<uint64_t>(ptr) >> 8;
  }

  /** * immediate size (3 bits) **/
  static constexpr auto ImmediateSize(TagPtr ptr) {
  ...
  /** * constant symbol T **/
  static constexpr auto T = static_cast<TagPtr>(
      ('t' << 8) | ((1 & 0x7) << 5) |
      ((static_cast<uint8_t>(IMMEDIATE_CLASS::KEYWORD) & 0x3) << 3) |
      (static_cast<uint8_t>(TAG::IMMEDIATE) & 0x7));

  /** * constant symbol NIL **/
  static constexpr auto NIL = static_cast<TagPtr>(
