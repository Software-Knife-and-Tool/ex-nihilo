...
/********
 **
 **  heapformat.h: libmu heap
 **
 **/
#if !defined(LIBMU_HEAPFMT_H_)
#define LIBMU_HEAPFMT_H_
...
class HeapFmt {
 ...
 public: /* tag */
  explicit HeapFmt(size_t extra, Tag ptr) {
    ...
    auto hi = reinterpret_cast<H*>(hImage_->data() + 1);

    *hi = *reinterpret_cast<H*>(Type::Untag<uint64_t>(ptr));
    tag_ = Type::Entag(hImage_->data() + 1, Type::TAG::EXTEND);
  }
