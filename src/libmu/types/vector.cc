...
Vector::Vector(Env* env, std::vector<TagPtr> src) {
  auto vp = env->heap_alloc<Layout>(
                                    sizeof(Layout) + (src.size() * sizeof(TagPtr)), SYS_CLASS::VECTOR, "vector:general");
...
Vector::Vector(Env* env, std::string src) {
  if (src.size() <= IMMEDIATE_STR_MAX) {
    ...
    auto vp = env->heap_alloc<Layout>(
                                      sizeof(Layout) + (src.size() * sizeof(char)), SYS_CLASS::STRING, "vector:char");
...
Vector::Vector(Env* env, std::vector<uint8_t> src) {
  auto vp = env->heap_alloc<Layout>(
                                    sizeof(Layout) + (src.size() * sizeof(uint8_t)), SYS_CLASS::VECTOR, "vector:byte");
...
Vector::Vector(Env* env, std::vector<int64_t> src) {
  auto vp = env->heap_alloc<Layout>(
                                    sizeof(Layout) + (src.size() * sizeof(int64_t)), SYS_CLASS::VECTOR, "vector:fixnum");
...
Vector::Vector(Env* env, std::vector<float> src) {
  auto vp = env->heap_alloc<Layout>(
                                    sizeof(Layout) + (src.size() * sizeof(float)), SYS_CLASS::VECTOR, "vector:float");
